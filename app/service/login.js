...
class LoginService extends Service {
  ...
  async login(username, password) {
    const result = await this.app.mysql.get('tb_users', { username, password });
    if (username === 'admin') {
      return {
        role: [ 'admin' ],
        name: 'admin',
  ...
  async LoginByEmail(email, password) {
    const result = await this.app.mysql.get('tb_users', { email, password });
    return {
      error_code: result ? 0 : 1,
      data: result,
    };
  ...
  async isHaveUserEmail(email) {
    const result = await this.app.mysql.get('tb_users', { email });
    return result;
  }
  // 判断用户名是否存在
  async isHaveUserName(username) {
    const result = await this.app.mysql.get('tb_users', { username });
    return result;
  }
}
